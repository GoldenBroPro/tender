    ...
    public function createTender(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'deposit.required' => 'Поле "Сумма задатка" обязательно для заполнения',
                'estimate_cost.required' => 'Поле "Начальная цена" обязательно для заполнения',
                'location.required' => 'Поле "Адрес" обязательно для заполнения',
                'category_id.required' => 'Выберете категорию',
                'tender_status.exists' => 'Выберете статус',
                'attachment.file' => 'Файл не загружен правильно',
                'category_id.exists' => 'Выберете категорию',
            ]);
            if ($validator->fails()) {
            ...
            session()->flash('message', "Успешно выполнено");
    ...
    public function updateTender(Request $request)
    {
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'title.required' => ':attribute поле, обязательное для заполнения.',
                'description.required' => ':attribute поле, обязательное для заполнения.',
                'start_date.required' => 'Требуется дата начала.',
                'end_date.required' => 'Требуется дата окончания.',
                'deposit.required' => 'Требуется сумма депозита.',
                'estimate_cost.required' => 'Оценка стоимости требуется.',
                'location.sometimes' => 'Место не может быть пустым',
                'category_id.required' => 'Категория требуется.',
                'tender_status.exists' => 'должен быть действительный статус',
            ...
            if ($tender->tm_tender_status_id !== config("global.tender_draft")) {
                throw new Exception("Только черновые тендеры могут обновлять");
            }
            ...
            if ($hasAttachment) {
                if ($attachment_path !== "") {
                    ...
                    throw new Exception("Если вы выберете, есть опция вложения, вы должны добавить вложение");
                }
            } else {
    ...
    public function createTenderCategory(Request $request)
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'name.required' => ':attribute поле, обязательное для заполнения.',
                'icon.required' => ':attribute поле, обязательное для заполнения.',
            ]);
            ...
            session()->flash('message', "успешно сохранено");
            session()->flash('flash_message_type', config("global.flash_success"));
    ...
    public function deleteTenderCategory(Request $request, $id)
    {
        try {
            ...
            if (count($tenderCategory->tenders()->get()) > 0) {
                throw new Exception("имеет прикрепленные тендеры");
            } else {
